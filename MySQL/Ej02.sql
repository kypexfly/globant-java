...
SELECT f.nombre, p.nombre, p.precio FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre IN ('Asus', 'Hewlett-Packard');
...
SELECT nombre FROM fabricante
	WHERE ( SELECT COUNT(*) FROM producto WHERE codigo_fabricante = fabricante.codigo ) 
